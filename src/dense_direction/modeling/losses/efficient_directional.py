...
class EfficientDirectionalLoss(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        direction_bins = torch.arange(0, div) / div
    ...
    @torch.no_grad()
    def _transform_gt_sem_seg(self, gt_sem_seg: Tensor, kernel_mask: Tensor) -> Tensor:
        """
        ...
        map_patches = F.unfold(
            gt_sem_seg_reshaped, self.k_size, padding=self.pad
        )  # n*k, k_size^2, h * w
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            gt_sem_seg (Tensor): Per class ground truth semantic segmentation map of
                shape (N, K, 1, H, W).
            weight (float | None): Optional weight for directional loss value.
