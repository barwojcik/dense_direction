...
class BaseDirectionDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        *,
        ...
        act_cfg: ConfigType | None = None,
        in_index: int | Sequence[int] = -1,
        input_transform: str | None = None,
        loss_decode: ConfigType | None = None,
        ignore_index: int = 255,
        sampler: ConfigType | None = None,
        ...
        Args:
            in_channels (int|Sequence[int]): Input channels.
            channels (int): Channels after modules, before conv_dir.
            dir_classes (Sequence[int]): Index or sequence of indexes of classes for which
            ...
            loss_decode (dict | Sequence[dict]): Config of decode loss.
                The `loss_name` is property of the corresponding loss function which could be shown
                in the training log. If you want this loss item to be included in the backward
                graph, `loss_` must be the prefix of the name. Defaults to 'loss_dir'.
                e.g. dict(type='DirectionalLoss'),
                [dict(type='DirectionalLoss', loss_name='loss_dir'),
                dict(type='SmoothnessLoss', loss_name='loss_smt')]
                Default: dict(type='DirectionalLoss').
            ...
            pre_norm_vectors (bool): Whether apply L2 normalization to directional vectors before
                vector filed resize. Default: False.
            post_norm_vectors (bool): Whether apply L2 normalization to directional vectors in
                output vector filed. Default: False.
            gt_scale_factor (float): Per class sematic segmentation map resize factor for loss
                calculation. Default: 1.
            init_cfg (dict or list[dict], optional): Initialization config dict.
        """
    ...
    def extra_repr(self):
        """Extra repr."""
        s = f"input_transform={self.input_transform}, align_corners={self.align_corners}"
        return s
    ...
    def _init_conv_dir(self):
        ...
        self.dropout = nn.Dropout2d(self.dropout_ratio) if self.dropout_ratio > 0 else None
    ...
    def _transform_inputs(self, inputs: list[Tensor]) -> Tensor | list[Tensor]:
        ...
        if self.input_transform == "resize_concat":
            in_index: list[int] = (
                [self.in_index] if isinstance(self.in_index, int) else list(self.in_index)
            )
            inputs: list[Tensor] = [inputs[i] for i in in_index]  # type: ignore[no-redef]
            ...
            inputs: Tensor = torch.cat(upsampled_inputs, dim=1)  # type: ignore[no-redef]

        elif self.input_transform == "multiple_select":
            in_index: list[int] = (  # type: ignore[no-redef]
                [self.in_index] if isinstance(self.in_index, int) else list(self.in_index)
            )
            inputs: list[Tensor] = [inputs[i] for i in in_index]  # type: ignore[no-redef]
            ...
            inputs: Tensor = inputs[self.in_index]  # type: ignore[no-redef]
    ...
    def predict(
        ...
        Args:
            inputs (tuple[Tensor]): List of multi-level img features.
            batch_img_metas (list[dict]): List Image info where each dict may also contain:
    ...
    def _stack_batch_gt(self, batch_data_samples: SampleList) -> Tensor:
        ...
        if self.gt_scale_factor != 1.0:
            per_class_gt_sem_seg: Tensor = self._resize_per_class_gt_sem_seg(per_class_gt_sem_seg)  # type: ignore[no-redef]
    ...
    def loss_by_feat(self, dir_vectors: Tensor, batch_data_samples: SampleList) -> dict:
        ...
        if not isinstance(self.loss_decode, nn.ModuleList):
            ...
            losses_decode = self.loss_decode  # type: ignore[assignment]
    ...
    def predict_by_feat(self, dir_vectors: Tensor, batch_img_metas: list[dict]) -> Tensor:
        ...
        h2, w2 = size
        dir_vectors = resize(
            input=dir_vectors.view(n, k * c, h1, w1),
            size=size,
            mode="bilinear",
            align_corners=self.align_corners,
        ).reshape(n, k, c, h2, w2)
