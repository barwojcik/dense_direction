...
This module provides a MultiscaleLossDirectionHead class. This class is a wrapper that wraps a
"real" direction head and any number of "dummy" direction heads. The "real" head performs like
all other direction heads, while the "dummy" heads enable loss computation with different
gt_scale_factor values.
"""
...
class MultiscaleLossDirectionHead(nn.Module):
    ...
    Args:
        ...
        dummy_heads (ConfigType | list[ConfigType]): The config or list of configs for the "dummy"
            direction heads. If not all keys are provided, they will be filled with the "real"
            values.
    """
    ...
    def __init__(
        ...
        Args:
            real_head (ConfigType): The config for the "real" direction head.
            dummy_heads (ConfigType | list[ConfigType]): The config or list of configs for the
                "dummy" direction heads. If not all keys are provided, they will be filled with
                the "real" values.
        """
        super().__init__()
    ...
    def loss(
        ...
        for head_idx, dummy_head in enumerate(self.dummy_heads):
            ...
            dummy_losses = {
                f"dir{head_idx + 2}.{key}": value for key, value in dummy_losses.items()
            }
