...
from mmseg.utils import (
    ConfigType, OptConfigType, OptMultiConfig, OptSampleList, SampleList, add_prefix,
)
    ...
    def postprocess_result(self, dir_vector_field: Tensor, data_samples: OptSampleList = None) -> SampleList:
        ...
        for i in range(batch_size):
            if not only_prediction:
                ...
                i_dir_vfield = dir_vector_field[i:i + 1, :,
            ...
            data_samples[i].set_data({
                'estimated_vs':
                    PixelData(**{'data': i_dir_vfield}),
                'estimated_dirs':
                    PixelData(**{'data': i_dir_angles})
            })
