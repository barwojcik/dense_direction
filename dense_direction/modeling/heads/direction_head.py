...
class BaseDirectionDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def predict(self, inputs: Tuple[Tensor], batch_img_metas: List[dict],
        ...
        Args:
            inputs (Tuple[Tensor]): List of multi-level img features.
            batch_img_metas (dict): List Image info where each dict may also contain: 'img_shape',
    ...
    def _trasnform_gt_sem_seg(self, gt_sem_seg: Tensor) -> Tensor:
        """
        Transforms gt_sem_seg maps into separate binary maps per class for which directions will
        be estimated.

        Args:
            gt_sem_seg (Tensor): Ground truth semantic segmentation map of shape (C, H, W).

        Returns:
            Tensor: Semantic segmentation map of shape (K, C, H, W) where K number of linear classes.
        """
        class_maps = []
    ...
    def _stack_batch_gt(self, batch_data_samples: SampleList) -> Tensor:
        ...
        gt_semantic_segs = [
            self._trasnform_gt_sem_seg(gt_sem_seg) for gt_sem_seg in gt_semantic_segs
        ]
